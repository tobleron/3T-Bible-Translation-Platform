...
class ProjectPaths:
    ...
    def __init__(
        ...
        if workspace_dir is None:
            workspace_dir = Path(__file__).resolve().parents[2] / "02_Human_Editorial_Workspace"
        if repo_root is None:
            repo_root = workspace_dir.parent

        self.workspace_dir = workspace_dir
        self.repo_root = repo_root
        self.bible_dir = self.repo_root / "03_EPUB_Production" / "_HOLY_BIBLE"
        self.justifications_dir = (
            self.repo_root / "03_EPUB_Production" / "_HOLY_BIBLE_JUSTIFICATIONS"
        )
        self.source_dirs = [
            self.repo_root / "01_AI_Translation_Engine" / "001_Flat_Bibles",
            self.workspace_dir / "flat_bibles",
        ]
        self.legacy_prompt_path = (
            self.repo_root / "01_AI_Translation_Engine" / "instructions_bible_crafter_prompt.txt"
        )
        self.state_dir = self.workspace_dir / ".ttt_workbench"
        ...
        self.lexical_db_path = self.workspace_dir / "data" / "lexical_index" / "lexical.db"
