...
def test_code_block_prompt_renders_with_new_label_and_default_text(monkeypatch) -> None:
    monkeypatch.setenv("TTT_WEBAPP_FAKE_LLM", "1")
    reset_controller()
    with TestClient(appmod.app) as client:
        response = client.get("/workspace/old/genesis/1/1-5")
        try:
            assert response.status_code == 200
            assert 'id="prompt-mode-copyable"' in response.text
            assert ">Code block</span>" in response.text
            assert "Return your response in a plain text code block." in response.text
        finally:
            response.close()
    reset_controller()
