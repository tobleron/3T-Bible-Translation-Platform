...
────────────────────────────
Convert a MyBible .bbl.mybible or .bbli SQLite database to a flat JSON for analysis.

USAGE
-----
    python mybible_to_flat_json.py lsv.bbli
    # Optional output filename:
    python mybible_to_flat_json.py lsv.bbli output.json
"""
...
# Possible table/column variations
TABLE_CANDIDATES = [
    ("bible",      ["book", "chapter", "verse", "scripture"]),
    ("Bible",      ["Book", "Chapter", "Verse", "Scripture"]),
    ("Bible",      ["book", "chapter", "verse", "scripture"]),   # rare, but some bbli use this
    ("bible",      ["Book", "Chapter", "Verse", "Scripture"]),   # rare, but just in case
]

def find_table_and_columns(cursor):
    ...
    for table, cols in TABLE_CANDIDATES:
        if table in tables:
            try:
                ...
                if all(col in actual_cols for col in cols):
                    return table, cols
            except Exception:
                continue
    return None, None
...
def convert_mybible_to_flat(input_path: Path, output_path: Path):
    ...
    table, cols = find_table_and_columns(cursor)
    if not table:
        print("❌ Could not find a suitable verses table with expected columns in this file.")
        conn.close()
        sys.exit(1)

    sql = f"SELECT {', '.join(cols)} FROM {table} ORDER BY {cols[0]}, {cols[1]}, {cols[2]}"
    cursor.execute(sql)
    flat_output = []
...
if __name__ == "__main__":
    if not (2 <= len(sys.argv) <= 3):
        sys.exit("Usage: python mybible_to_flat_json.py <input.bbl.mybible or .bbli> [output.json]")
