...
class EpubCommandsMixin:
    ...
    def cmd_epub_gen(self: WorkbenchApp, args: list[str]) -> None:
        ...
        t0 = time.monotonic()
        cmd = [sys.executable, "generate_epub.py", "--md", "--txt"]
        try:
            result = subprocess.run(
                cmd, cwd=output_dir, capture_output=True, text=True, check=False
            )
        ...
        if "ModuleNotFoundError" in combined:
            ...
            lines.append(
                ...
                "03_EPUB_Production/INSTALL.txt before rerunning /epub-gen."
            )
        if outputs:
