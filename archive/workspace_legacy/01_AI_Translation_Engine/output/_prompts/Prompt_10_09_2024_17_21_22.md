Convert the following script to a bash shell script.
...
{
    if (Test-Path $promptInputFile)
    {
        Clear-Content $promptInputFile
    } else
    {
        New-Item $promptInputFile -ItemType File
    }
}
...
Add-Content -Path $promptInputFile -Value "**$UserName**`:`n$userQuestion"
